...
public class StarPosition
{
 ...
 /*
  T=2000年からの経過ユリウス年
  d=日本時間における0時からの経過時間。日の少数 (0<d < 1.0)
  rhamda=観測地経度
 */
 ...
 public static double getSunDirection(double asc ,double dec, double lat, double phai )
  {
   double t = (phai - asc) / 180.0 * Math.PI; //時角
   double latitude = lat / 180.0 * Math.PI; //緯度ラジアン変換
   double declination = dec / 180.0 * Math.PI;//赤緯ラジアン変換

   double A = Math.atan((-Math.cos(declination) * Math.sin(t)) / (Math.sin(declination)* Math.cos(latitude) - Math.cos(declination) * Math.sin(latitude) * Math.cos(t)) );
   return A;
  }
 //太陽の高度
